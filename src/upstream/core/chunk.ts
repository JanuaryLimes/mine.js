...
class Chunk {
  ...
  constructor(
    ...
    { size, dimension, maxHeight }: ChunkOptions
  ) {
    this.size = size;
    ...
    this.voxels = ndarray(pool.mallocUint8(size * maxHeight * size), [
      size,
      maxHeight,
      size,
    ]);
  ...
  distTo(vx: number, _: number, vz: number) {
    ...
    return Math.sqrt(
      (mx + this.size / 2 - vx) ** 2 + (mz + this.size / 2 - vz) ** 2
    );
  }
  ...
  setupMesh(meshDataList: ServerMeshType[]) {
    ...
    meshDataList.forEach((meshData) => {
      MESH_TYPES.forEach((type) => {
        ...
        geometry?.setAttribute(
          "normal",
          new Int8BufferAttribute(normals, normalNumComponents)
        );
        geometry?.setAttribute(
          "uv",
        ...
        geometry?.setAttribute(
          "ao",
          new Float32BufferAttribute(aos, occlusionNumComponents)
        );
        geometry?.setAttribute(
          "sunlight",
          new Float32BufferAttribute(sunlights, sunlightsNumComponents)
        );
        geometry?.setAttribute(
          "torchLight",
          new Float32BufferAttribute(torchLights, torchLightsNumComponents)
        );
