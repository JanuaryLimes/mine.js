...
class Player {
  ...
  get lookBlockStr() {
    ...
    return lookBlock
      ? `${lookBlock[0]} ${lookBlock[1]} ${lookBlock[2]}`
      : "None";
  }
  ...
  private updateLookBlock() {
    ...
    const result = raycast(
      (x, y, z) =>
        Boolean(
          world.getVoxelByWorld([Math.floor(x), Math.floor(y), Math.floor(z)])
        ),
    ...
    const newLookBlock = Helper.mapWorldPosToVoxelPos(
      <Coords3>flooredPoint,
      world.options.dimension
    );
