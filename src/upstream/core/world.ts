...
class World extends EventEmitter {
  ...
  getVoxelByWorld(wCoords: Coords3) {
    const vCoords = Helper.mapWorldPosToVoxelPos(
      wCoords,
  ...
  setRenderRadius(renderRadiuus: number) {
    ...
    registry.opaqueChunkMaterial.uniforms.uFogNear.value =
      renderRadiuus * 0.6 * chunkSize * dimension;
    registry.opaqueChunkMaterial.uniforms.uFogFar.value =
      renderRadiuus * chunkSize * dimension;
  ...
  private surroundCamChunks() {
    ...
    this.pendingChunks = Array.from(
      new Set(this.pendingChunks.map((pc) => Helper.getChunkName(pc)))
    ).map((pcStr) => Helper.parseChunkName(pcStr) as Coords2);

    // make pending chunks radiate from player, might have easier ways of doing so
    this.pendingChunks.sort(
      (a, b) =>
        (cx - a[0]) ** 2 +
        (cz - a[1]) ** 2 -
        (cx - b[0]) ** 2 -
    ...
    const visibleChunksArray = Array.from(this.visibleChunks);
    for (const chunk of visibleChunksArray) {
  ...
  private requestChunks() {
    ...
    if (this.requestedChunks.size < maxChunkRequestPerFrame) {
      const framePendingChunks = this.pendingChunks.splice(
        0,
        maxChunkRequestPerFrame
      );
  ...
  private meshChunks() {
    ...
    const frameReceivedChunks = this.receivedChunks.splice(
      0,
      maxChunkProcessPerFrame
    );
    frameReceivedChunks.forEach((serverChunk) => {
      ...
      if (!chunk) {
        ...
        chunk = new Chunk(this.engine, coords, {
          size: chunkSize,
          dimension,
          maxHeight,
        });
